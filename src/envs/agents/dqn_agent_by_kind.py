...
class ExperienceBufferByKind(ExperienceBuffer):
    ...
    def encode_states(self, states, return_tensors=True):
        ...
        for state in states:
            ...
            for kind, (features_list, dir_list) in state_by_kind.items():
...
class DQNAgentByKind(DQNAgent):
    def __init__(self, state_type, action_space_n,
        ...
        entity_kinds = [token.split('_')[0] for token in ENTITY_TOKENS]
        entity_kinds = list(set(entity_kinds))  # Remove duplicates
