...
def test_calculate_output_np(explorers, wanderers, mock_env):
    ...
    solver = PPOConvExtSolver(info, EXTENDED_KUTULU_ACTIONS, weights, size=agent.size, explicit_action_mask=explicit_action_mask)
...
class TestPPOAgentConvExt:
    def test_convert_ppo_conv_ext(self, explorers, wanderers, mock_env):
        agent = PPOAgent(
            state_type='conv_ext',
            actions=EXTENDED_KUTULU_ACTIONS,
            model_params={'size': 3},
    ...
    def test_ppo_conv_ext_model_structure(self, explorers, wanderers, mock_env):
        """Test that PPO conv_ext model produces valid outputs with correct structure"""
        agent = PPOAgent(
            state_type='conv_ext',
            actions=EXTENDED_KUTULU_ACTIONS,
            model_params={'size': 3},
        ...
        # Note: 'is_occupied' is only present if include_occupation_head=True in model params
    ...
    def test_ppo_conv_ext_vs_conv(self, explorers, wanderers, mock_env):
        """Test that conv_ext and conv models produce different outputs"""
        # Create conv_ext and conv agents
        conv_ext_agent = PPOAgent(
            state_type='conv_ext',
            actions=EXTENDED_KUTULU_ACTIONS,
            model_params={'size': 3},
            train=False,
        ...
        for agent in [conv_ext_agent, conv_agent]:
            agent.set_env(mock_env)
            agent.observer.env._set_entities(entities)
            agent.observer.env._set_players(entities, set_ids=True)
        
        conv_ext_state = conv_ext_agent.observer.get_state(0)
        conv_state = conv_agent.observer.get_state(0)
        
        conv_ext_tensor = conv_ext_agent.episode_buffer.encode_states([conv_ext_state])
        conv_tensor = conv_agent.episode_buffer.encode_states([conv_state])
        
        conv_ext_agent.model.eval()
        conv_agent.model.eval()
        
        with torch.no_grad():
            conv_ext_output = conv_ext_agent.model(conv_ext_tensor)
            conv_output = conv_agent.model(conv_tensor)
        
        # Both should produce valid outputs
        conv_ext_policy = conv_ext_output['policy'][0]
        conv_policy = conv_output['policy'][0]
        
        assert torch.all(torch.isfinite(conv_ext_policy))
        assert torch.all(torch.isfinite(conv_policy))
        assert conv_ext_policy.shape == conv_policy.shape == torch.Size([len(EXTENDED_KUTULU_ACTIONS)])
        
        # Check that both are valid probability distributions
        assert torch.allclose(conv_ext_policy.sum(), torch.tensor(1.0), atol=1e-6)
        assert torch.allclose(conv_policy.sum(), torch.tensor(1.0), atol=1e-6)

    def test_backward_compatibility_ppo_conv_ext(self, explorers, wanderers, mock_env):
        """Test that PPO conv_ext agent still works as expected"""
        agent = PPOAgent(
            state_type='conv_ext',
            actions=EXTENDED_KUTULU_ACTIONS,
            model_params={'size': 3},
    ...
    def test_ppo_conv_ext_action_generation(self, explorers, wanderers, mock_env):
        """Test that PPO conv_ext agent can generate actions properly"""
        agent = PPOAgent(
            state_type='conv_ext',
            actions=EXTENDED_KUTULU_ACTIONS,
            model_params={'size': 3},
